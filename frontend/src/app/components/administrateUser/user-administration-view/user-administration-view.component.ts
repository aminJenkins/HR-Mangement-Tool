...
export class UserAdministrationViewComponent {
  ...
  displayedColumns: string[] = ['name','nachname','abteilung','email','delete'];
  ...
  constructor(private userAdministrationService: userAdministrationService, private router: Router, public dialog: MatDialog,private snackbar: MatSnackBar) {
  }
  ...
  public loadData(): void {
    this.userAdministrationService.getDatasource().subscribe((response: EmployeeExist[]) => {
      this.dataSource = response;
      console.log("Empoyees: "+response);
    });
    this.userAdministrationService.getDepartments().subscribe((response: Department []) =>{
  ...
  public delete(element: any):void{
    ...
    this.userAdministrationService.deleteEmployee(element.id);
