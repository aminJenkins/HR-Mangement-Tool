...
public interface TerminRepo extends JpaRepository<TerminEntity, Integer> {
    ...
    @Query(value = "SELECT * FROM TERMIN WHERE ERSTELLER_ID = :userId AND DATUM = :startDate" +
            " UNION ( SELECT T.* FROM TERMIN T JOIN TERMIN_TEILNEHMER TD  ON T.ID = TD.TERMIN_ID " +
            "WHERE MITARBEITER_ID =:userId AND T.DATUM = :startDate) ORDER BY BEGINN ASC", nativeQuery = true)
